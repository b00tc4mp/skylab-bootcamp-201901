...
var questions1 = [
    ...
    { letter: "d", answer: "debil", question: "CON LA D. Que tiene poca fuerza, poco vigor o poca resistencia."},
    ...
    { letter: "x", answer: "xilofono", question: "CON LA X. Instrumento musical de percusión compuesto por láminas (generalmente de madera) de diferentes tamaños, en orden similar al piano que emite diferentes sonidos al golpearlo con dos baquetas."},
...
var questions2 = [
    ...
    { letter: "d", answer: "dublin", question: "CON LA D. Ciudad capital de Irlanda."},
    ...
    { letter: "g", answer: "genero", question: "CON LA G. cualquier cosa que tomada en conjunto es objeto de comercio."},
    ...
    { letter: "u", answer: "urografia", question: "CON LA U. Radiografía de las vías urinarias."},
    ...
    { letter: "x", answer: "clinex", question: "CONTIENE LA X. Marca registrada que ha pasado a ser la denominación común del pañuelo de papel."},
    { letter: "y", answer: "anteayer", question: "CONTIENE LA Y. En el día que precede inmediatamente al de ayer."},
    { letter: "z", answer: "zarate", question: "CON LA Z. Apellido del escritor colombiano que ganó el Premio Planeta en 1972 con la obra 'La cárcel'."},
    ];
...
function runGame() {
    ...
    j=0;
    k=0;
    for(var l=0; l<questions0.length;l++){
        document.getElementById(allQ[pos][l].letter).style.background='#fff';
    }
    document.getElementById('pregunta').innerHTML = allQ[pos][i].question;
...
function pasQuestion(){
    ...
    pasAr[j]=allQ[pos][i];
    document.getElementById(allQ[pos][i].letter).style.background='#ffa500';
    pos = Math.floor(Math.random()*3);
    i++;
    j++;
    if(i<questions0.length){ 
        document.getElementById('pregunta').innerHTML = allQ[pos][i].question;
    }
    else{
        checkAnsw();
...
function checkAnsw(){
    ...
    if(score+fail<questions0.length && i<27){
        checkPreg();
    }
    else if(score+fail<questions0.length && i>=27 && k<j)
    {
        checkPasas();
            
    }else if(score+fail>=questions0.length)
    {
        jackpot(bote);
    }  
}
...
function checkPreg(){
    answ = document.getElementById('answcont').value;
        switch(answ.toLowerCase()){

            case allQ[pos][i].answer:

                document.getElementById(allQ[pos][i].letter).style.background='#8fc800';
                document.getElementById('answcont').value = '';
                score++;
                i++;
                break;
            ...
            default:
                ...
                document.getElementById(allQ[pos][i].letter).style.background='#cc0000';
                document.getElementById('answcont').value = '';
                fail++;
                i++;
                bote=false;
...
function  checkPasas(){
    document.getElementById('pregunta').innerHTML = pasAr[k].question;     
            answ = document.getElementById('answcont').value;
        ...
        if(!pasas){
            ...
            switch(answ.toLowerCase()){
                ...
                case pasAr[k].answer:
                    ...
                    document.getElementById(pasAr[k].letter).style.background='#8fc800';
                    document.getElementById('answcont').value = '';
                    score++;        
                    break;
                ...
                default:
                    ...
                    document.getElementById(pasAr[k].letter).style.background='#cc0000';
                    document.getElementById('answcont').value = '';
                    fail++;
                    bote=false;
            ...
            k++;
            if(k>=pasAr.length){jackpot(bote);
            }
            else{
            document.getElementById('pregunta').innerHTML = pasAr[k].question;
            }
...
function jackpot(pot){
    invisible('answb');
    visible('info');
    pot ? document.getElementById('info').innerHTML ="ENHORABUENA CAMPEÓN! TE LLEVAS EL BOTE":document.getElementById('info').innerHTML ="LÁSTIMA! TUS RESULTADOS EN "+(score+fail)+" PREGUNTAS SON:\n"+score+" aciertos y "+fail+" fallos.";
    visible('startb');
}
