...
function Desktop({ handleState, handleNewFolder, handleNewFile, openDir, closeFinder, openFinderRoot, openMenu, logOut, handleLogout, openFile, closeFile, dragStart, openFileFromFinder, updatePath, closeFeedback, dragzoneError, openCalc, changeItems }) {
    ...
    let [itemsChanged, setItemsChanged] = useState(null)
    let desktop = useRef()
    let pathFromFinder
    ...
    //     debugger
    //     if (itemsChanged) {
    //         return logic.retrieveLevel(actualPath)
    //             .then(content => {
    //                 setFinder(content)
    //             })
    //             .catch(err => {
    ...
    dragzoneError = (err) => {
        if (err === "Unexpected end of JSON input") {
            err = "There has been an error while connecting"
        }
        setFeedback(true)
        setFeedbackError(err)
    }
    ...
    return <section className="desktop" id="desktop" ref={desktop}>
        ...
        <div className="desktop__toolbar">
            <Toolbar newFolder={handleNewFolder} newFile={handleNewFile} openFinder={openFinderRoot} openMenu={openMenu} openCalc={openCalc}></Toolbar>
        </div>
        <div className="desktop__dragzone">
            <Dragzone dir={level} pos={positions} openDir={openDir} openFile={openFile} dragItem={finderItem} setDragzoneError={dragzoneError} changeItems={changeItems}></Dragzone>
        </div>
        {
