...
function Finder({ content, close, dragStart, dragChanged, openFolder, openFile, openFileFromFinder, onDragStart, onDrag, dragEnd, actualFinderPath, finderFeedback, changePath }) {
    ...
    let [draggedChanged, setDraggedChanged] = useState(null)
    let posX
    let posY

    dragChanged = e => {
        setDraggedChanged(!draggedChanged)
        dragStart(e)
    }
    ...
    return <section className="finder" id="finder" draggable ref={finder} onDrag={e => onDrag(e)} onDragStart={e => onDragStart(e)} onDragEnd={dragEnd}>
                ...
                finderContent.children.length > 0 ?
                    <section className="finder__list">
                        {finderContent.children.map((item, index) => {
                            if (item.type === "folder") {
                                return <div className="finder__item" key={index} onClick={(e) => openFolder(e)}>
                                ...
                                return <div className="finder__item" key={index} onClick={(e) => openFile(e)}>
                    ...
                    <section className="finder__dragzone">
                        {
                            finderContent.children.map((item, index) => {
                                if (item.type === "folder") {
                                    return <div className="finder__dragzone__item" id={item.type} key={index} draggable onDragStart={e => dragChanged(e)}>
