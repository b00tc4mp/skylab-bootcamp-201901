...
const FileStore = require('session-file-store')(session) //li dius q les session les guardi a aqui
const logicFactory = require('./src/logic-factory')


// cookie es unica per sessió, no x usuari.nomes transporta id (del storage q esta en memoria. nomes viatja un sesion id)
const { argv: [, , port = 8080] } = process

const app = express()

app.use(session({  //es un middleware (sempre q hi hagi use()es un middlewere i s'executa entre el )
...
const formBodyParser = bodyParser.urlencoded({ extended: false }) //If extended is false, you can not post "nested object" //// Nested Object = { person: { name: cw } }

app.use(express.static('public')) // es un middlewere. diu q miri el css abans de renderitzar l'html


function pullFeedback(req) {  //resseteja el feedback
    const { session: { feedback } } = req //ens guardem el feedback pq x cada usuari, hi pot haver un missatge
...
app.get('/', (req, res) => {  //(app.get('/',----aqui s'executa el middlewere--- (req, res) )
