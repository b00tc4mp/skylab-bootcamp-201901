...
class MapForm extends Component {
      ...
      logic
        ...
        .then(pmap => {
          ...
          this.setState({
            ...
            mapInfo: pmapInfo,
            coverImage: pmapInfo.coverImage || null,
    ...
    (async () => {
      try {
        ...
        if (Object.keys(validateErrors).length !== 0) {
          ...
          if (!this.state.mapId) {
            const resp = await logic.createMap(
              e.target.mapTitle.value,
              this.state.description,
              e.target.coverImage.value
            );
            this.setState({ mapId: resp.id, redirectToMap: true });
          } else {
            const data = {};
  ...
  handleCancelCreate = () => {
    this.setState(this.state.mapId ? { redirectToMap: true } : { redirectToHome: true });
  };
              ...
              <form className="uk-form-stacked" onSubmit={this.handleSubmit}>
                <div className="uk-margin">
                  ...
                  {this.state.errors.title && (
                    <span className="uk-text-danger uk-margin-medium-left">{this.state.errors.title}</span>
                  )}
                  <div className="uk-form-controls">
                    <input
                      ...
                      type="string"
                      name="mapTitle"
                      placeholder={mapTitlePlaceholder}
                      defaultValue={this.state.mapInfo.mapTitle}
                      autoFocus
                      required
                    />
                  </div>
                ...
                <div className="uk-margin">
                  ...
                  {this.state.errors.coverImage && (
                    <span className="uk-text-danger uk-margin-medium-left">{this.state.errors.coverImage}</span>
                  )}
                  <div className="uk-form-controls">
                    <input
                      className="uk-input uk-form-width-large"
                      ...
                      defaultValue={this.state.mapInfo.coverImage}
                ...
                <div className="uk-flex uk-flex-center">
                  <button className="custom-form-button" type="submit">
                    {this.state.mapId ? update : create}
                  </button>
                  <button className="custom-form-button" type="button" onClick={this.handleCancelCreate}>
                    {cancel}
                  </button>
                </div>
              </form>
