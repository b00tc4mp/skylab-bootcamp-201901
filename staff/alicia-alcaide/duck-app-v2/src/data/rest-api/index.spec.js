    ...
    describe('ducks', () => {
            ...
            it('should succeed adding fav on first time', () =>
                restApi.toggleFavDuck(token, duckId)
                    .then(response => {
                        ...
                        expect(message).toBe('Ok, duck toggled to favorites.')
                    })
            ...
            it('should succeed adding duck to cart on first time', () =>
                restApi.toggleCartDuck(token, duckId)
                    .then(response => {
                        ...
                        expect(message).toBe('Ok, duck toggled to cart.')
                    })
        ...
        describe('retrieve cart ducks', () => {
                ...
                return restApi.searchDucks(token, '')
                    .then(ducks => {
                        ...
                        for (let i = 0; i < 10; i++) {
                            ...
                            toggles.push(restApi.toggleCartDuck(token, duckId))
                        }
            ...
            it('should succeed on cart', () =>
                restApi.retrieveCartDucks(token)
                    .then(ducks => {
                        ducks.forEach(({ id, title, imageUrl, description, price }) => {
                            const isInCart = _cart.some(cart => cart === id)
