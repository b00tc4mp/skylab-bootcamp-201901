    ...
    return (
        <header className="nav-bar">
            ...
            <div className='nav-bar__right'>
                {props.history.location.pathname === '/restaurant-results' && !showUserButton && <div className='nav-bar__right-desksearch'><Search setQuery={props.setQuery}/></div>}
                {props.history.location.pathname === '/restaurant-results' && <button onClick={e => {e.preventDefault(); props.history.push('/event-categories')}} className={`nav-bar__${eventsButton}`}>events</button>}
                {props.history.location.pathname === ('/event-categories' || '/events-nearme' || '/events-map') && <button onClick={e => {e.preventDefault(); props.history.push('/restaurant-results')}} className={`nav-bar__${eventsButton}`}>+ new event</button>}
                {props.history.location.pathname !== ('/event-categories' || '/events-nearme' || '/events-map' || '/restaurant-results') && <div className='nav-bar__two-buttons'><button onClick={e => {e.preventDefault(); props.history.push('/event-categories')}} className={`nav-bar__${eventsButton}`}>events</button><button onClick={e => {e.preventDefault(); props.history.push('/restaurant-results')}} className={`nav-bar__${eventsButton}`}>+ new event</button></div>}
                ...
                {props.history.location.pathname === '/restaurant-results' && showUserButton && <i className={`fas fa-search nav-bar__${searchButton}`} onClick={e => {e.preventDefault(); setSearch('search'); setSearchButton('no-search-button'); setLogo('no-logo'); setUserButton('no-user-button'); setEventsButton('no-events-button')}}></i>}
