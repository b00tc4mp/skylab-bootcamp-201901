...
const logic = {
    ...
    */
    ...
    addCommenttoArtist(artistId, comment) {
        if (typeof artistId !== 'string') throw TypeError(`${artistId} is not a string`)
        if (!artistId.trim().length) throw Error('artistId is empty')
        if (typeof comment !== 'string') throw TypeError(`${comment} is not a string`)
        if (!comment.trim().length) throw Error('comment is empty')

        return userApi.addCommenttoArtist(this.__userId__,this.__userApiToken__,artistId, comment)
    },

    retrieveCommentsfromArtist(artistId) {
        if (typeof artistId !== 'string') throw TypeError(`${artistId} is not a string`)
        if (!artistId.trim().length) throw Error('artistId is empty')


        return userApi.retrieveCommentfromArtist(artistId)
    },
