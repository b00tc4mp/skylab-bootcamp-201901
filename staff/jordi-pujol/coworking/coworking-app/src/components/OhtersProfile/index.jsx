import React, { Component } from 'react'
import { withRouter } from 'react-router-dom'
import logic from '../../logic';
import Feedback from '../Feedback'

class OthersProfile extends Component {

    state = { name: '', surname: '', email: '', companyName: '', username: '', time: null, image: '', age: '', feedback: null, origin: null }

    componentWillMount() {

        const { props: { username, origin } } = this

        try{
            logic.retrieveUserProfile(username)
                .then(({ name, surname, email, companyName, age, username, time, image }) => {
                    this.setState({ name })
                    this.setState({ surname })
                    this.setState({ email })
                    this.setState({ companyName })
                    this.setState({ age })
                    this.setState({ username })
                    this.setState({ time })
                    this.setState({ image })
                })
                .catch(({message}) => {
                    this.setState({feedback: message})
                    this.props.history.push('/home/inbox')})
        } catch ({message}){
            this.setState({feedback: message})
        }

        this.setState({origin})   
    }

    handleCloseModal = () => {

        const {state: {origin}} = this

        if (origin == 'inbox'){
            this.props.history.push('/home/inbox')
        }
        else {
            this.props.history.push('/home/myservices/submited')
        }
    }

    render() {

        const { state: { name, surname, email, companyName, age, image, username, time, feedback }, handleCloseModal } = this

        return <section className="otherProfile">
            <section className="profile">
                <div className="profile__content">
                    <form className="profile__content--form">
                        <i onClick={handleCloseModal} className="fas fa-window-close fa-2x"></i>
                        {image ? <img className="profile__content--form--userImage" src={image} /> : <img className="profile__content--form--userImage" src="data:image/png;base64," />}
                        <div className="profile__content--form--inputs">
                            <input className="profile__content--name" type="text" value={name} disabled />
                            <input className="profile__content--company" type="text" value={companyName} disabled />
                        </div>
                    </form>
                    <div className="profile__info">
                        <form className="profile__form">
                            <div className="profile__form--item">
                                <p>Name:</p>
                                <input className="profile__form--info" type="text" value={name} disabled />
                            </div>
                            <div className="profile__form--item">
                                <p>Surname:</p>
                                <input className="profile__form--info" type="text" value={surname} disabled />
                            </div>
                            <div className="profile__form--item">
                                <p>Username:</p>
                                <input className="profile__form--info" type="text" value={username} disabled />
                            </div>
                            <div className="profile__form--item">
                                <p>My Time:</p>
                                <input className="profile__form--info" type="text" value={time} disabled />
                            </div>
                            <div className="profile__form--item">
                                <p>Email:</p>
                                <input className="profile__form--info" type="email" value={email} disabled />
                            </div>
                            <div className="profile__form--item">
                                <p>Company Name:</p>
                                <input className="profile__form--info" type="text" value={companyName} disabled />
                            </div>
                            <div className="profile__form--item">
                                <p>Age:</p>
                                <input className="profile__form--info" type="text" value={age} disabled />
                            </div>
                        </form>
                    </div>
                </div>
                {feedback && <Feedback message={feedback} />}
            </section>
        </section>
    }
}

export default withRouter(OthersProfile)