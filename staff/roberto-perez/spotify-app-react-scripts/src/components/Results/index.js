...
class Results extends Component {
  render() {
    const {
      props: { title, results, onItemClick }
    } = this;
    ...
    return (
      <div className="container">
        ...
        <div className="results row">
          {results &&
            results.map(({ id, name, type, images: [{ url: image } = {}] }) => (
              <div key={id} className="col-xs-6 col-sm-4 col-md-3 col-lg-2 col-xl-2">
                <div className="result" onClick={() => onItemClick(id, name, image)}>
                  <div className={(type === 'artist') ? 'result__img-container result__img-container--radius' : 'result__img-container'}>
                    ...
                    {image && (<div className={(type === 'artist') ? 'result__image result__image--radius' : 'result__image'} style={{ backgroundImage: "url(" + image + ")" }} />)}
                  </div>
                  <header className="result__header">
                    <h4 className="result__name">{name}</h4>
                  </header>
                </div>
              </div>
            ))}
