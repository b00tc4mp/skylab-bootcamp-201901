...
function ExpandButton(props) {
    ...
    const animationTo = useRef(null);
    const animationFrom = useRef(null);
    const expandButton = useRef(null);

	const fullScreen = () => {
		if (expandButton.current.classList.contains('animated')) {
			document.exitFullscreen();
			expandButton.current.classList.remove('animated');
			animationFrom.current.beginElement();
		} else {
			props.hostGame.current.requestFullscreen();
			expandButton.current.classList.add('animated');
			animationTo.current.beginElement();
		}
	};

	return (
		<button className="expand_button" ref={expandButton} onClick={fullScreen}>
